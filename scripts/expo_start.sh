...
case $operation_str in
  ...
  4) tree -L 1 -a ${HOKUYO_NAV2_PKG_PATH}/rosbag ; echo "rosbagディレクトリを入力してください" ; read inbagname ; echo "p2oで使うrosbag名を入力してください" ; read p2obagname ; cd ${HOKUYO_NAV2_PKG_PATH}; scripts/get_rosbag.bash rosbag/${inbagname} rosbag/${p2obagname}; exit;;
  5) tree -L 1 -a ${HOKUYO_NAV2_PKG_PATH}/rosbag ; echo "p2oで使うrosbagディレクトリを入力してください" ; read p2obagname ; echo "出力したい地図の名前を入力してください" ; read p2omapname ; cd ${HOKUYO_NAV2_PKG_PATH}; scripts/hokuyo_slam.bash ${p2obagname} ${p2omapname}; exit;;
